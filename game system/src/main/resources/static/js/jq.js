...
function buttonXls(data,permission,pers) {
	if(permission != ""){
		if ($.inArray(permission, pers) < 0) {
			return "";
		}
	}
	var btn = $("<button class='layui-btn layui-btn-xs' title='导出' onclick='exportXls(\"" + data +"\")'><i class='layui-icon'>&#xe62d;</i></button>");
...
function buttonDels(data, permission, pers){
	...
	var btn = $("<button class='layui-btn layui-btn-xs' title='删除' onclick='dels(\"" + data +"\")'><i class='layui-icon'>&#xe640;</i></button>");
	return btn.prop("outerHTML");
}
